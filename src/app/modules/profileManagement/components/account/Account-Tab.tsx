...
const AccountTab = () => {
  ...
  // Helper function to safely convert string to AccountType
  const safeAccountType = (accountType?: string): AccountType => {
    if (accountType === "Company" || accountType === "Agent" || accountType === "Individual User") {
      return accountType as AccountType;
    }
    return "Individual User"; // Default fallback
  }
  ...
  // Sync with Redux state when userData changes - FIXED TYPE ISSUE
  useEffect(() => {
    setFormValues((prev) => ({
      ...prev,
      ...
      accountType: safeAccountType(userData.accountType) || prev.accountType, // Fixed: Use safeAccountType helper
    }))
  }, [userData])
