...
// Create a function to generate a verification URL
const generateVerificationUrl = (data: CertificateData) => {
  // Base URL of your application
  const baseUrl = window.location.origin;
  
  // Create URL with certificate info in query parameters
...
export const generateCertificate = async (data: CertificateData) => {
  ...
  // Generate QR code with a verification URL instead of JSON data
  const qrSize = 50; // in mm
  const qrX = 105 - qrSize/2;
  const qrY = 225;
  
  try {
    // Generate a verification URL instead of JSON data
    ...
    // Add text under QR code explaining its purpose
    doc.setFontSize(8);
    doc.text('Scan to verify certificate authenticity', 105, qrY + qrSize + 5, { align: 'center' });
  } catch (error) {
    console.error('Error generating QR code:', error);
    // Fallback: Draw empty QR code placeholder
    doc.rect(qrX, qrY, qrSize, qrSize);
  ...
  // Date at bottom
  doc.setFontSize(10);
  doc.text(`This certificate was generated on ${new Date().toLocaleDateString('en-US', { 
    month: 'long', 
    day: 'numeric', 
    year: 'numeric' 
  })}`, 105, 280, { align: 'center' });
